...
class BurpReporter:
    ...
    async def generate(self, scan_data: dict[str, Any], output_dir: Path) -> Path:
        ...
        for secret in secrets:
            ...
            xml_parts.append("    <type>5244416</type>")
            ...
            xml_parts.append("    <confidence>Firm</confidence>")
            xml_parts.append("    <issueBackground>An exposed secret or API key was found.</issueBackground>")
            xml_parts.append("    <remediationBackground>Remove the secret and revoke the compromised credentials.</remediationBackground>")
        ...
        for ssl in ssl_issues:
            ...
            xml_parts.append("  <issue>")
            xml_parts.append(f"    <serialNumber>{serial}</serialNumber>")
            xml_parts.append("    <type>16842752</type>")
            xml_parts.append(f"    <name>SSL certificate: {issue_title}</name>")
            xml_parts.append(f"    <host>https://{hostname}</host>")
            xml_parts.append("    <path>/</path>")
            xml_parts.append("    <location>/</location>")
            xml_parts.append(f"    <severity>{severity}</severity>")
            xml_parts.append("    <confidence>Certain</confidence>")
            xml_parts.append("    <issueBackground>A potential SSL/TLS configuration issue was found.</issueBackground>")
            xml_parts.append("    <remediationBackground>Ensure the server is configured with strong ciphers and valid certificates.</remediationBackground>")
        ...
        for port in open_ports:
            ...
            xml_parts.append("  <issue>")
            xml_parts.append(f"    <serialNumber>{serial}</serialNumber>")
            xml_parts.append("    <type>0</type>")
            xml_parts.append(f"    <name>Open port {p_num} ({service})</name>")
            xml_parts.append(f"    <host>{host}</host>")
            xml_parts.append("    <path>/</path>")
            xml_parts.append("    <location>/</location>")
            xml_parts.append("    <severity>Information</severity>")
            xml_parts.append("    <confidence>Certain</confidence>")
            xml_parts.append("    <issueBackground>An open network port was detected.</issueBackground>")
            xml_parts.append("    <remediationBackground>Ensure this port is intended to be exposed.</remediationBackground>")
        ...
        for chain in attack_chains:
            ...
            xml_parts.append("    <location>/</location>")
            xml_parts.append(f"    <severity>{severity}</severity>")
            xml_parts.append("    <confidence>Tentative</confidence>")
            xml_parts.append(f"    <issueBackground>{desc}</issueBackground>")
            xml_parts.append("    <remediationBackground>Investigate and resolve the reported issue.</remediationBackground>")
